...
const generateUniqueId = (length: number) => {
    ...
    let emojiId = '';
    const randomSalt = Math.floor(Math.random() * 100000).toString();
    ...
    for (let i = 0; i < length; i++) {
        ...
        // Occasionally add a piece of the salt or time to the sequence
        if (i % 3 === 0) {
            emojiId += emojis[Math.floor(Math.random() * emojis.length)];
        }
    }

    // Append a hashed version of the salt and time for extra uniqueness
    const uniqueHash = `${randomSalt}${currentTime}`;
    for (let char of uniqueHash) {
        const randomIndex = Math.floor(Math.random() * emojis.length);
        emojiId += emojis[randomIndex];
    }
...
const encryptText = (content:string)=>{
    ...
    if(content.length > 100){
        sequenceLenght = Math.floor(content.length/3);
    }
   
     return generateUniqueId(sequenceLenght);
}

const encryptMsg = async (formData: FormData) => {
    const content = formData.get('normaltext') as string;
    const password = formData.get('password') as string;
    let encryption = encryptText(content) as string;
